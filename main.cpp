...
struct Address {
    ...
    {
        return ar(JMKVP(street), JMKVP(number), JMKVP(test_field), JMKVP(nullable_string), JMKVP(another_nullable_string));
    }
};
...
struct Student {
    ...
    bool SerializeToJson(Archiver& ar) const
    {
        return ar(JMKVP(address), JMKVP(name), JMKVP(age), JMKVP(scores));
    }
};
...
int main(int argc, char** argv)
{
    ...
    stu.address = {
        { "Home", { { "ShangHai", "China", "Asia" }, 1234, {}, pStr.get(), nullptr } },
        { "School", { { "Beijing", "China", "Asia" }, 1234, { 123, 2.5 }, nullptr, std::make_shared<std::string>("Another nullable string") } }
    };
    stu.name = "Bill";
    stu.age = 44;
    stu.scores = {
